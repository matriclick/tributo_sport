  ...
  def create
    ...
    respond_to do |format|
      if @contact_type.save
        format.html { redirect_to @contact_type, :notice => "#{t('activerecord.successful.messages.created', :model =>  @contact_type.class.model_name.human)}"  }
  ...
  def update
    ...
    respond_to do |format|
      if @contact_type.update_attributes(params[:contact_type])
        format.html { redirect_to @contact_type, :notice => "#{t('activerecord.successful.messages.updated', :model =>  @contact_type.class.model_name.human)}"  }
        format.json { head :ok }
      else
